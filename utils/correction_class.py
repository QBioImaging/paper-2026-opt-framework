...
import logging
import numpy as np
from utils.utils_opt import norm_max, img_to_int_type, is_positive
...
class Correct:
    ...
    def correctBadPxs(self, img: np.array, mode: str = "n4") -> np.array:
        ...
        for badPx in self.badPxs:
            ...
            if np.issubdtype(ans.dtype, np.floating):
                # if the image is float, use np.mean
                ans[badPx] = np.mean(neigh_vals)
            else:
        ...
        if np.issubdtype(ans.dtype, np.integer):
            # cast it on correct dtype
            ans = img_to_int_type(ans, dtype=out_type)
        else:
            ans = ans.astype(out_type, copy=False)
        return ans
    ...
    def correct_dark_bright(
        ...
        if modality == "Transmission":
            if useBright and useDark:
                ...
                stack = self.subtract_images(stack, self.dark)
            else:
            ...
            if useBright:
                ...
                stack = self.subtract_images(stack, self.bright)
            ...
            neg_count = np.sum(stack < 0)
            if neg_count > 0:
                self.logger.warning(f"Found {neg_count} negative values in the stack. Will clip them")

                # get rid of potential negative values
                stack = np.clip(stack, 0, None).astype(np.uint16)
    ...
    def clip_data(self, data_corr: np.ndarray) -> np.ndarray:
        ...
        data_corr = data_corr.astype(np.float16, copy=False)
        return data_corr
